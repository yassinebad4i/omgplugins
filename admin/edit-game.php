...
$gameId = (int)$_GET['id'];
$game = $json->getItem('games', $gameId);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Security::verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($title) || empty($version) || empty($download_url)) {
            ...
            if ($json->slugExists('games', $slug, $gameId)) {
                ...
                if ($status === 'published' && !$game['published_at']) {
                    $updateData['published_at'] = date('Y-m-d H:i:s');
                } elseif ($status === 'published') {
                    $updateData['published_at'] = $game['published_at'];
                } else {
                    $updateData['published_at'] = null;
                }
                
                if ($json->updateItem('games', $gameId, $updateData)) {
...
<head>
    ...
    <title>Edit Game - OMGPlugins CMS</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <header class="header">
        <h1 class="header-title">Edit Game</h1>
    </header>
        ...
        <div class="form-container">
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="title" name="title" value="<?php echo Security::escape($game['title'] ?? ''); ?>" required>
                </div>
                
                <div class="form-row">
                    <div class="form-group">
                        <label for="version">Version <span class="required">*</span></label>
                        <input type="text" id="version" name="version" value="<?php echo Security::escape($game['version'] ?? ''); ?>" required>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="category" name="category">
                            ...
                            <?php foreach ($categories as $cat): ?>
                                <option value="<?php echo Security::escape($cat['name']); ?>" <?php echo (($game['category'] ?? '') === $cat['name']) ? 'selected' : ''; ?>>
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description"><?php echo Security::escape($game['description'] ?? ''); ?></textarea>
                </div>
                
                <div class="form-group">
                    <label for="features">Features (comma-separated)</label>
                    <textarea id="features" name="features"><?php echo Security::escape($game['features'] ?? ''); ?></textarea>
                </div>
                
                <div class="form-group">
                    <label for="download_url">Download URL <span class="required">*</span></label>
                    <input type="url" id="download_url" name="download_url" value="<?php echo Security::escape($game['download_url'] ?? ''); ?>" required>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="badge_type" name="badge_type">
                            ...
                            <option value="new" <?php echo (($game['badge_type'] ?? '') === 'new') ? 'selected' : ''; ?>>New</option>
                            <option value="hot" <?php echo (($game['badge_type'] ?? '') === 'hot') ? 'selected' : ''; ?>>Hot</option>
                            <option value="featured" <?php echo (($game['badge_type'] ?? '') === 'featured') ? 'selected' : ''; ?>>Featured</option>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="status" name="status">
                            ...
                            <option value="published" <?php echo (($game['status'] ?? '') === 'published') ? 'selected' : ''; ?>>Published</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="meta_title" name="meta_title" value="<?php echo Security::escape($game['meta_title'] ?? ''); ?>">
                </div>
                
                <div class="form-group">
                    <label for="meta_description">Meta Description (SEO)</label>
                    <textarea id="meta_description" name="meta_description"><?php echo Security::escape($game['meta_description'] ?? ''); ?></textarea>
                </div>
